#![cfg_attr(
    feature = "nightly",
    feature(try_trait_v2, maybe_uninit_slice)
)]
...
//! let context = hostfxr.initialize_for_dotnet_command_line(common::test_dll_path()).unwrap();
...
//! let hostfxr = nethost::load_hostfxr().unwrap();
//! let context =
//!     hostfxr.initialize_for_runtime_config(common::test_runtime_config_path()).unwrap();
//! let fn_loader =
//!     context.get_delegate_loader_for_assembly(common::test_dll_path()).unwrap();
